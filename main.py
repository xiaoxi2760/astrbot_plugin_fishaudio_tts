...
调用本工具后，不要在最终文字回复中重复 text 参数的内容；其他对话内容正常以文字回复。

Args:
    text(string): 需要朗读的完整内容。该内容只会作为音频发送，不会作为普通文字发送；不要在最终文字回复中重复它。
...
"""
    ...
    async def on_llm_request_boost(self, event: AstrMessageEvent, req: ProviderRequest):
        ...
        req.system_prompt += (
            ...
            "调用 tts_speak 后，不要再把 text 参数的内容重复成文字；其余对话内容仍以正常文字回复。"
        )
